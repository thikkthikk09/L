...
;(function (global) {
  ...
  async function discoverProxy() {
    ...
    if (isStaticHosting() && !hasJwtForPayment() && getBakongEmail() && getRegisterCode()) {
      try {
        await renewJwtDirect()
      } catch (e) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          try {
            ...
            serverHasJwt = Boolean(health.hasJwt) || hasJwtForPayment()
          } catch {
            serverHasJwt = hasJwtForPayment()
          }
    ...
    try {
      ...
      if (!resolveProxyUrl() && isBrowser()) {
        setPaymentStatus(
          'pending',
          isGitHubPages()
            ? 'Payment API: run start.bat locally OR deploy this repo to Vercel'
            : 'Run start.bat then open http://127.0.0.1:8787/index.html',
        )
        return
      }
      ...
      if (err.message === 'PROXY_OFFLINE') {
        setPaymentStatus(
          'pending',
          isGitHubPages()
            ? 'Cannot reach payment server — use start.bat or Vercel deploy'
      ...
      if (err.message === 'Failed to fetch' || err.name === 'TypeError') {
        setPaymentStatus(
          'pending',
          isGitHubPages()
            ? 'Network blocked — deploy to Vercel or use http://127.0.0.1:8787'
            : 'Waiting for payment… (is start.bat running?)',
        )
        return
      }
